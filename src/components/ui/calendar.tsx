...
function Calendar({
  ...
  // Custom formatters for Arabic
  const arabicFormatters = {
    formatWeekdayName: (date: Date) => {
      return date.toLocaleDateString(isEnglish ? 'en-US' : 'ar', {
        weekday: 'narrow',
      });
    },
  ...
  return (
    <DayPicker
      dir={isEnglish ? 'ltr' : 'rtl'} // Set direction based on language
          ...
          if (orientation === 'left') {
            return (
              <ChevronLeftIcon
                className={cn(
                  'size-4',
                  !isEnglish && 'rotate-180', // Flip in RTL
                  className,
                )}
                {...props}
              />
