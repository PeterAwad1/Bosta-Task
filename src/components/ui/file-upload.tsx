        ...
        if (rejectedFiles.length > 0) {
          for (let i = 0; i < rejectedFiles.length; i++) {
            ...
            if (rejectedFiles[i].errors[0]?.code === 'file-invalid-type') {
              toast.error(t('fileTypeNotAllowed')); // <-- add this translation
              break;
            }

            if (rejectedFiles[i].errors[0]?.message) {
              toast.error(t('fileTooMany'));
              break;
            }
          }
        }
...
>(({ className, index, children, ...props }, ref) => {
  ...
  return (
    <div
      ...
      {...props}
    >
      <div className='flex h-full w-full items-center justify-between gap-1.5 font-medium leading-none tracking-tight'>
        {children}
      </div>
      <button
        type='button'
